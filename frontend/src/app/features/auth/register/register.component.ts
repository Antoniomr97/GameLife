...
@Component({
    ...
    <div class="auth-container">
      <div class="auth-card">
        <div class="auth-header">
          ...
          <h1>Únete a GameLife</h1>
          <p>Crea tu cuenta y empieza a reseñar</p>
        </div>
        <form (ngSubmit)="onRegister()" class="auth-form">
          <div class="form-group">
            <label for="username">Nombre de usuario</label>
            <input id="username" type="text" [(ngModel)]="username" name="username"
                   placeholder="gamer_pro" required minlength="3" />
          </div>
          <div class="form-group">
  ...
  styles: [`
    ...
    .auth-header h1 { font-size: 1.5rem; font-weight: 700; color: #e2e8f0; margin: 0 0 0.5rem; }
    ...
    .success-msg {
      ...
      color: #86efac; padding: 0.6rem 1rem; border-radius: 8px;
      font-size: 0.85rem; margin-bottom: 1rem;
    }
    .btn-full { width: 100%; }
    .btn-primary {
      padding: 0.75rem; background: linear-gradient(135deg, #8b5cf6, #6d28d9);
      color: white; border: none; border-radius: 10px;
      font-size: 0.95rem; font-weight: 600; cursor: pointer; transition: all 0.2s;
    }
    .btn-primary:hover:not(:disabled) { background: linear-gradient(135deg, #7c3aed, #5b21b6); transform: translateY(-1px); }
    .btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }
    .auth-footer { text-align: center; margin-top: 1.5rem; color: #94a3b8; font-size: 0.85rem; }
